...
use app\admin\model\Dcate;
use app\admin\model\Download as pro;
...
class Download extends Base
{
    ...
    {
        //现在用all后期需要调整和修改
        $res=pro::field('id,pid,title,downlink,downcount,image,update_time')->order('id','desc')->select();
        //dump($res);
        $this->assign('pro',$res);
        return $this->fetch();
    }
    ...
    public function catecreate()
    {
        ...
        // $cate=Dcate::field('catename,id,pid')->select();
        $cate=Dcate::getCate();
    ...
    public function catesave()
    {
        ...
        if (Request::isAjax()) {
            ...
            $sus=Dcate::where('catename',$acate['catename'])->find();
            if ($sus) {
                # code...
                return['sus'=>0,'msg'=>'类目已存在'];
            }else{
                $res=Dcate::create(Request::post());
